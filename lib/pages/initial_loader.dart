...
class InitialLoader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        body: const Center(child: const CircularProgressIndicator.adaptive()));
  }
}
...
void checkAuthState(BuildContext context) =>
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      if (authData.isLoggedIn)
        await _navigateToHomePage(context, provider, authData.token);
      else
  ...
  final route = MaterialPageRoute(
    builder: (context) => MultiProvider(providers: [
      ChangeNotifierProvider(
        create: (_) => UserProvider(authProvider),
      ),
      ChangeNotifierProvider(
          create: (_) => BooksProvider(context, authProvider)),
